...
const BashTool: ToolDefinition = {
  ...
  async execute(args, cwd) {
    ...
    return new Promise<ToolResult>((resolve) => {
      // 민감 환경변수 필터링 — exact match + wildcard 패턴 + 부분 매칭
      // TOKEN, SECRET, KEY, PASSWORD, CREDENTIAL, AUTH를 어디서든 포함하면 차단
      const SENSITIVE_PATTERNS = /(?:TOKEN|SECRET|_KEY|PASSWORD|CREDENTIAL|AUTH|PRIVATE|DATABASE_URL|CONNECTION_STRING)|(?:^(?:AWS_|GCP_|AZURE_|GOOGLE_|HEROKU_|VERCEL_|NETLIFY_|FIREBASE_|SUPABASE_|NPM_CONFIG_))/i;
